...
(function ( $ ) {
	...
	window.VcTabsView = vc.shortcode_view.extend({
        ...
        createAddTabButton:function () {
            ...
            this.$add_button = $('<li class="add_tab_block"><a href="#new-tab-' + new_tab_button_id + '" class="add_tab" title=""></a></li>').appendTo(this.$tabs.find(".tabs_controls"));
        },
        addTab:function (e) {
            e.preventDefault();
    ...
    window.VcTabView = window.VcColumnView.extend({
        ...
        deleteShortcode:function (e) {
            ...
            var params = this.model.get('params'),
                current_tab_index = $('[href=#tab-' + params.id + ']', this.$tabs).parent().index();
	...
	window.VcTabsView35 = vc.shortcode_view.extend( {
		...
		addTab: function ( e ) {
			...
			if(this.model.get('shortcode') === 'testimonial_slider') tab_title = window.i18nLocale.testimonial;
		...
		changedContent: function ( view ) {
			...
			if ( view.model.get( 'cloned' ) === true ) {
				var cloned_from = view.model.get( 'cloned_from' );
				var index = $('#tab-' + cloned_from.params.id).index();
                this.$tabs.tabs("add", "#tab-" + params.id, params.title, index - 1);
				this.$tabs.tabs( "select", index - 1 );
			} else {
				this.$tabs.tabs("add", "#tab-" + params.id, params.title, this.$tabs.tabs("length") - 1);
		...
		cloneModel: function ( model, parent_id, save_order ) {
			...
			if (model.get('shortcode') === 'tab') _.extend(new_params, {id:+new Date() + '-' + this.$tabs.tabs('length') + '-' + Math.floor(Math.random() * 11)});
            model_clone = Shortcodes.create({shortcode:model.get('shortcode'), id:vc_guid(), parent_id:parent_id, order:new_order, cloned:(model.get('shortcode') === 'tab' ? false : true), cloned_from:model.toJSON(), params:new_params})
